 ...
 */
/** Returns true when a Stripe error means the customer ID no longer exists. */
function isCustomerMissing(err: unknown): boolean {
  return (
    ...
    ((err as Record<string, unknown>).message as string).toLowerCase().includes("customer")
  );
}
...
class StripePaymentsProvider implements PaymentsProvider {
  async createTopUpCheckout(params: CreateTopUpParams): Promise<CreateTopUpResult> {
    ...
    // Resolve customer — create fresh if missing or if the stored ID is stale.
    ...
    try {
      ...
      // Stale customer ID (e.g. from a previous test/mock environment) — create fresh.
...
/**
 * Returns a fresh Stripe client via the Replit connector (falls back to
 * STRIPE_SECRET_KEY env var for local dev without the connector).
 * Never cached — the connector can rotate keys at any time.
 */
...
function createPaymentsProvider(): PaymentsProvider {
  ...
  // Credentials now come from the Replit Stripe connector (getStripe() above).
  // No STRIPE_SECRET_KEY env var check needed.
  return new StripePaymentsProvider();
}

// Lazy singleton — provider is created on the first method call, not at module
...
function getPaymentsProvider(): PaymentsProvider {
  if (!_paymentsProvider) _paymentsProvider = createPaymentsProvider();
  return _paymentsProvider;
}
export const payments: PaymentsProvider = {
  createTopUpCheckout: (params) => getPaymentsProvider().createTopUpCheckout(params),
  createCardSetupCheckout: (params) => getPaymentsProvider().createCardSetupCheckout(params),
  chargeSavedCard: (params) => getPaymentsProvider().chargeSavedCard(params),
