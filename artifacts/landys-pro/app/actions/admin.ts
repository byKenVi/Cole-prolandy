...
/**
 * Clear the Clerk user link for a contractor so they can re-claim their
 * account on next sign-in. Use this when a contractor's Clerk instance was
 * replaced (e.g. migrating from a personal Clerk account to Replit-managed
 * Clerk) and the old clerkUserId is now stale.
 */
export async function resetContractorClerkLink(contractorId: string): Promise<Result> {
  const admin = await requireAdmin();
  const contractor = await prisma.contractor.findUnique({
    where: { id: contractorId },
  ...
  await prisma.auditLog.create({
    data: {
      actorType: "admin",
      actorId: admin.email,
      action: "contractor.clerk.unlinked",
      targetType: "Contractor",
      targetId: contractorId,
      metadata: { previousClerkUserId: contractor.clerkUserId, reason: "admin_reset" },
    },
  });
